...
class TestIsPublicIp:
    ...
    def test_is_public_ip(self, ip, expected):
...
class TestNormalizeHttpUrl:
    ...
    def test_normalize_http_url_rejects_unsafe_syntax(self, url):
        with pytest.raises(UnsafeUrlError):
            normalize_http_url(url)


class TestPinnedSyncTransport:
    def test_pinned_sync_transport_dials_validated_ip_after_dns_flips(
        self, monkeypatch
    ):
        monkeypatch.setattr(socket, "getaddrinfo", _fake_getaddrinfo("93.184.216.34"))
...
class TestPinnedTransport:
    @pytest.mark.anyio
    async def test_pinned_transport_dials_validated_ip_after_dns_flips(
        self, monkeypatch
    ):
