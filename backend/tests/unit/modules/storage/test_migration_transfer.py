...
def test_unbounded_consumer_read_remains_bounded():
...
def test_chunk_adapter_preserves_every_byte():
...
def test_changed_source_fails_before_end_of_transfer(payload, size, digest, code):
    reader = ThrottledReader(
...
def test_low_bandwidth_never_sleeps_for_more_than_one_quarter_second(monkeypatch):
    now = [0.0]
    delays = []
...
def test_pause_checkpoint_stops_before_consuming_more_source_bytes():
