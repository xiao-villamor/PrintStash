...
def test_empty_journal_allows_normal_startup(journal):
    assert migration_journal.events() == []
    assert not migration_journal.inspect_before_writes()


def test_completed_other_run_cannot_mask_interrupted_copy(journal):
...
def test_malformed_record_holds_maintenance(journal, record):
    journal.write_text(record)
    assert migration_journal.inspect_before_writes()
    assert restore_in_progress()


def test_parallel_receipts_remain_individually_readable(journal):
    entries = [
        {"run_id": "copy", "nonce": "1", "phase": "copy_receipt", "object_id": index}
...
def test_completed_activation_with_first_write_is_resolved(journal):
    for phase in ("activation_intent", "database_active", "complete", "first_write"):
