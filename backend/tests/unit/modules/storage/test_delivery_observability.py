...
class TestDeliveryObservability:
    def test_counts_consumed_proxy_bytes(self):
    ...
    def test_releases_metered_stream_on_disconnect(self):
        released = []
    ...
    def test_redacts_unknown_delivery_labels(self):
    ...
    def test_closed_proxy_stream_stays_closed(self):
        stream = telemetry.MeteredChunks(iter([b"unused"]), "s3", "download")
        stream.close()

        assert list(stream) == []

    def test_propagates_provider_stream_failure(self):
        import pytest
    ...
    def test_keeps_proxy_bytes_when_metrics_fail(self, monkeypatch):
    ...
    def test_keeps_delivery_when_strategy_metrics_fail(self, monkeypatch):
        def fail(*args, **kwargs):
            raise RuntimeError("metrics unavailable")
