"""Defends ``test_printer_profile_crud`` behavior for the ``v1`` production unit.
...
def test_printer_profile_create_duplicate_name_conflict(
    ...
    first = client.post("/api/v1/printer-profiles", headers=auth_headers, json=payload)
    assert first.status_code == 201
    second = client.post("/api/v1/printer-profiles", headers=auth_headers, json=payload)
...
def test_printer_profile_delete_success_and_not_found(
    ...
    resp = client.delete(f"/api/v1/printer-profiles/{profile_id}", headers=auth_headers)
