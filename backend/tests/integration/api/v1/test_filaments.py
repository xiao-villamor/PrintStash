"""Defends ``test_filament_profile_crud`` behavior for the ``v1`` production unit.
...
def test_create_duplicate_name_conflict(
    ...
    first = client.post("/api/v1/filament-profiles", headers=auth_headers, json=payload)
...
def test_update_not_found(client: TestClient, auth_headers: dict[str, str]) -> None:
    resp = client.patch(
...
def test_delete_not_found(client: TestClient, auth_headers: dict[str, str]) -> None:
    resp = client.delete("/api/v1/filament-profiles/999", headers=auth_headers)
    assert resp.status_code == 404
    assert resp.json()["detail"] == "filament_profile_not_found"
