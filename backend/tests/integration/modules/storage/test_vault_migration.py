...
@pytest.fixture
def migration_disk_db(tmp_path):
    """Concurrent writers use production WAL, not shared-cache table locks."""
...
class TestSourceConfig:
    def test_legacy_s3_environment_preserves_provider_identity(
        self, db_session, monkeypatch
    ):
        import json

        from app.modules.storage.vault_migration import _source_config

        monkeypatch.setitem(_overlay, "storage_backend", "s3")
        monkeypatch.setitem(_overlay, "s3_bucket", "fake-source-bucket")
        monkeypatch.setitem(_overlay, "s3_endpoint_url", "https://storage.example.test")
        monkeypatch.setitem(_overlay, "s3_region", "us-east-1")
        monkeypatch.setitem(_overlay, "s3_access_key", "fake-access")
        monkeypatch.setitem(_overlay, "s3_secret_key", "fake-secret")
        monkeypatch.setitem(_overlay, "s3_root", "owned-prefix")
        monkeypatch.setitem(_overlay, "storage_provider_config", "")

        config = json.loads(_source_config(db_session))

        assert config["provider"] == "s3"
        assert config["bucket"] == "fake-source-bucket"
        assert config["endpoint_url"] == "https://storage.example.test"
        assert config["root"] == "owned-prefix"
        assert config["access_key"] == "fake-access"
        assert config["secret_key"] == "fake-secret"

    def test_structured_environment_overrides_legacy_roots(
            ...
            json.dumps(
                {
                    ...
                    "thumb_dir": str(tmp_path / "configured-thumbs"),
                }
            ),
        )
        ...
        assert config["data_dir"] == str(tmp_path / "configured-data")
        assert config["thumb_dir"] == str(tmp_path / "configured-thumbs")

    def test_unavailable_source_configuration_is_not_guessed(
        self, db_session, monkeypatch
    ):
        from app.modules.storage.vault_migration import _source_config

        monkeypatch.setitem(_overlay, "storage_backend", "unknown")
        monkeypatch.setitem(_overlay, "storage_provider_config", "")

        with pytest.raises(
            ValueError, match="migration_source_configuration_unavailable"
        ):
            _source_config(db_session)


class TestLocalStorageBackend:
    def test_local_backend_retains_its_roots_when_configuration_changes(
        self, tmp_path: Path
    ):
        ...
        assert backend.blob_key("model", 1, "part.stl") == str(
            source / "model/v1/part.stl"
        )
...
class TestBuildConfiguredBackend:
    ...
    def test_candidate_local_backend_never_changes_active_settings(
        ...
        _overlay.update(
            data_dir=tmp_path / "active", thumb_dir=tmp_path / "active-thumbs"
        )
...
class TestReadGeneration:
    def test_reader_keeps_source_adapter_after_activation(self, tmp_path: Path):
        from app.modules.storage.storage_backend import generations
    ...
    def test_activation_cannot_mix_an_unplanned_reader(self, tmp_path: Path):
        import pytest
...
class TestRetainStorageObjects:
    def test_retention_exemption_is_exactly_one_candidate(self, tmp_path: Path):
        import pytest
        ...
        with retain_storage_objects():
            with allow_retained_destination_destruction(
                ...
                for protected in (source, foreign):
                    with pytest.raises(
                        OperationError, match="storage_snapshot_retained"
                    ):
                        protected.remove()
...
class TestVaultMigrations:
    def test_provider_corruption_is_recreated_only_from_its_receipt(
        self, db_session, tmp_path, migration_owner, monkeypatch
    ):
        ...
        create_stream = LocalStorageBackend.create_stream

        def corrupt_write(backend, _stream, key):
            return create_stream(backend, BytesIO(b"corrupt!"), key)

        with monkeypatch.context() as fault:
            fault.setattr(LocalStorageBackend, "create_stream", corrupt_write)
            with pytest.raises(
                ValueError, match="migration_destination_verification_failed"
            ):
                migration_owner.advance(plan["id"])
        assert migration_owner.get(plan["id"])["failed_objects"] == 1
        migration_owner.resume(plan["id"], failed_only=True)
        ...
        assert Path(obj.destination_key).read_bytes() == b"original"
        assert source.read_bytes(artifact.path) == b"original"

    def test_unhealthy_committed_destination_keeps_recovery_gate(
        self, db_session, tmp_path, migration_owner, monkeypatch
    ):
        ...
        migration_owner.start(plan["id"], plan["plan_digest"])
        probe = LocalStorageBackend.health_probe

        def unavailable_destination(backend):
            return (
                probe(backend) if backend.data_dir == source.data_dir else {"ok": False}
            )

        with monkeypatch.context() as fault:
            fault.setattr(LocalStorageBackend, "health_probe", unavailable_destination)
            with pytest.raises(
                ValueError, match="migration_activated_destination_unhealthy"
            ):
                migration_owner.cutover(plan["id"])

        assert restore_in_progress()
        assert migration_owner.get(plan["id"])["state"] == "recovery_required"
        assert db_session.get(VaultGeneration, 1).activation_run_id == plan["id"]
        assert get_backend().data_dir != source.data_dir
        assert migration_owner.recover(plan["id"])["state"] == "active"
        assert not restore_in_progress()

    def test_preflight_is_unavailable_during_recovery(self, tmp_path, migration_owner):
        ...
        with pytest.raises(ValueError, match="migration_recovery_required"):
            migration_owner.preflight(candidate, backup_id="verified")

        assert list(Path(candidate["data_dir"]).iterdir()) == []
        assert migration_owner.list() == []

    def test_resume_cannot_bypass_recovery(self, tmp_path, migration_owner):
        from app.runtime.maintenance import hold_restore_maintenance
    ...
    def test_resume_rejects_a_changed_source_epoch(self, tmp_path, migration_owner):
        from app.modules.storage.storage_backend import generations
        from app.modules.storage.storage_backend.runtime import get_backend
        ...
        migration_owner.pause(plan["id"])
        with generations.activation():
            generations.publish(get_backend(), "other-source-epoch")

        with pytest.raises(ValueError, match="migration_source_generation_changed"):
            migration_owner.resume(plan["id"])

        assert migration_owner.get(plan["id"])["state"] == "paused"

    def test_source_cleanup_respects_retention_period(
        ...
        source = get_backend()
        artifact = build_stored_file(
            db_session, source, build_model(db_session), data=b"retained"
        )
        key = artifact.path
        plan = migration_owner.preflight(
            migration_candidate(tmp_path), backup_id="verified"
        ...
        with pytest.raises(ValueError, match="migration_source_grace_required"):
            migration_owner.cleanup(plan["id"], confirmation=plan["id"], source=True)

        assert source.read_bytes(key) == b"retained"

    @pytest.mark.parametrize(
        ("action", "code"),
        [
            ("pause", "migration_cannot_pause"),
            ("resume", "migration_cannot_resume"),
            ("advance", "migration_not_copying"),
            ("cutover", "migration_not_ready"),
            ("full_audit", "migration_destination_not_active"),
            ("retain", "migration_destination_not_active"),
        ],
    )
    def test_unstarted_plan_rejects_invalid_actions(
        self, tmp_path, migration_owner, action, code
    ):
        plan = migration_owner.preflight(
            migration_candidate(tmp_path), backup_id="verified"
        )
        ...
        with pytest.raises(ValueError, match=code):
            getattr(migration_owner, action)(plan["id"])

        assert migration_owner.get(plan["id"])["state"] == "planned"

    def test_history_returns_newest_persisted_plan(self, tmp_path, migration_owner):
        plan = migration_owner.preflight(
            migration_candidate(tmp_path), backup_id="verified"
        )

        assert [run["id"] for run in migration_owner.list()] == [plan["id"]]
        with pytest.raises(ValueError, match="migration_not_found"):
            migration_owner.get("unknown")

    def test_missing_destination_root_is_not_created(self, tmp_path, migration_owner):
        candidate = migration_candidate(tmp_path)
        Path(candidate["data_dir"]).rmdir()

        with pytest.raises(ValueError, match="migration_destination_root_unavailable"):
            migration_owner.preflight(candidate, backup_id="verified")

        assert not Path(candidate["data_dir"]).exists()
        assert migration_owner.list() == []

    def test_foreign_binding_is_not_replaced(self, tmp_path, migration_owner):
        import json

        candidate = migration_candidate(tmp_path)
        marker = Path(candidate["data_dir"]) / ".printstash-storage-root.json"
        content = json.dumps({"format": 1, "role": "data", "installation": "other"})
        marker.write_text(content)

        with pytest.raises(ValueError, match="migration_destination_binding_invalid"):
            migration_owner.preflight(candidate, backup_id="verified")

        assert marker.read_text() == content
        assert migration_owner.list() == []

    def test_unhealthy_source_cannot_enroll_candidate(
        self, tmp_path, migration_owner, monkeypatch
    ):
        from app.modules.storage.storage_backend.runtime import get_backend

        candidate = migration_candidate(tmp_path)
        monkeypatch.setattr(get_backend(), "health_probe", lambda: {"ok": False})

        with pytest.raises(ValueError, match="migration_source_unhealthy"):
            migration_owner.preflight(candidate, backup_id="verified")

        assert list(Path(candidate["data_dir"]).iterdir()) == []

    def test_capacity_rejection_discards_unstarted_claim(
        self, tmp_path, migration_owner, monkeypatch
    ):
        from app.core.errors import ErrorKind, OperationError

        def denied(*args, **kwargs):
            raise OperationError("capacity_unavailable", kind=ErrorKind.BUSY)

        monkeypatch.setattr(migration_owner.capacity, "reserve", denied)

        with pytest.raises(OperationError, match="capacity_unavailable"):
            migration_owner.preflight(
                migration_candidate(tmp_path), backup_id="verified"
            )

        run = migration_owner.list()[0]
        assert run["state"] == "discarded"
        assert run["error_code"] == "migration_capacity_unavailable"

    def test_expired_preflight_cannot_start(
        self, db_session, tmp_path, migration_owner
    ):
        ...
        run = db_session.get(VaultMigrationRun, plan["id"])
        run.expires_at = utcnow() - timedelta(seconds=1)
        db_session.add(run)
        db_session.commit()

        with pytest.raises(ValueError, match="migration_plan_stale"):
            migration_owner.start(plan["id"], plan["plan_digest"])

        assert migration_owner.get(plan["id"])["state"] == "planned"

    def test_changed_source_epoch_invalidates_preflight(
        self, tmp_path, migration_owner
    ):
        from app.modules.storage.storage_backend import generations
        from app.modules.storage.storage_backend.runtime import get_backend
        ...
        with generations.activation():
            generations.publish(get_backend(), "newer-epoch")
    ...
    def test_parallel_batch_verifies_both_objects(
        self, migration_disk_db, db_session, tmp_path, migration_owner
    ):
        from app.modules.storage.storage_backend.runtime import get_backend
        from tests.factories import build_model, build_stored_file
        ...
        first = build_stored_file(
            db_session, source, build_model(db_session), data=b"one"
        )
        second = build_stored_file(
            db_session, source, build_model(db_session), data=b"two"
        )
        plan = migration_owner.preflight(
            migration_candidate(tmp_path),
            backup_id="verified",
            policy={"concurrency": 2, "bandwidth_bytes_per_second": 1024},
        )
        migration_owner.start(plan["id"], plan["plan_digest"])
        ...
        assert source.read_bytes(first.path) == b"one"
        assert source.read_bytes(second.path) == b"two"

    def test_copy_receipt_journal_recovers_before_database_receipt(
        self, db_session, tmp_path, migration_owner, monkeypatch
    ):
        ...
        artifact = build_stored_file(
            db_session, get_backend(), build_model(db_session), data=b"durable"
        )
        ...
        migration_owner.start(plan["id"], plan["plan_digest"])
        append = migration_journal.append
        ...
        with monkeypatch.context() as fault:
            fault.setattr(migration_journal, "append", crash)
            with pytest.raises(Crash):
                migration_owner.advance(plan["id"])
        assert migration_journal.inspect_before_writes()
        assert migration_owner.recover(plan["id"])["state"] == "paused"
        migration_owner.resume(plan["id"])
        ...
        assert get_backend().read_bytes(original_key) == b"durable"

    def test_recovery_without_journal_holds_maintenance(
        self, tmp_path, migration_owner
    ):
        from app.runtime.maintenance import restore_in_progress
        ...
        with pytest.raises(ValueError, match="migration_recovery_ambiguous"):
            migration_owner.recover(plan["id"])

        assert restore_in_progress()
        assert migration_owner.get(plan["id"])["state"] == "planned"

    def test_recovery_nonce_mismatch_holds_maintenance(self, tmp_path, migration_owner):
        from app.modules.storage import migration_journal
        from app.runtime.maintenance import restore_in_progress
        ...
        migration_journal.append(
            {"phase": "paused", "run_id": plan["id"], "nonce": "other-run"}
        )
        ...
        assert restore_in_progress()
        assert migration_owner.get(plan["id"])["state"] == "copying"

    def test_destination_recovery_refuses_changed_activation_snapshot(
        self, db_session, active_copy
    ):
        from sqlmodel import select
        ...
        owner, plan, source, old_key = active_copy
        obj = db_session.exec(
            select(VaultMigrationObject).where(
                VaultMigrationObject.run_id == plan["id"]
            )
        ).one()
        Path(obj.destination_key).write_bytes(b"changed destination")

        with pytest.raises(ValueError, match="migration_recovery_destination_changed"):
            owner.recover(plan["id"])

        assert restore_in_progress()
        assert source.read_bytes(old_key) == b"retain"

    @pytest.mark.parametrize(
        "failure,code",
        [
            pytest.param(
                FileNotFoundError("fake-private-location"),
                "migration_source_missing",
                id="missing",
            ),
            pytest.param(
                OSError("fake-secret-provider-message"),
                "migration_storage_unavailable",
                id="provider-error",
            ),
        ],
    )
    def test_source_copy_failure_is_retryable_without_leaking_provider_error(
        self, db_session, tmp_path, migration_owner, monkeypatch, failure, code
    ):
        from app.modules.storage.storage_backend.runtime import get_backend
        from tests.factories import build_model, build_stored_file
        ...
        with monkeypatch.context() as fault:
            fault.setattr(LocalStorageBackend, "stream_chunks", unavailable)
            with pytest.raises(type(failure), match="fake-"):
                migration_owner.advance(plan["id"])
        ...
        assert get_backend() is source
        assert source.read_bytes(artifact.path) == b"source"

    def test_retry_failed_preserves_verified_objects(
        self, db_session, tmp_path, migration_owner, monkeypatch
    ):
        from sqlmodel import select
        ...
        source = get_backend()
        build_stored_file(db_session, source, build_model(db_session), data=b"one")
        build_stored_file(db_session, source, build_model(db_session), data=b"two")
        plan = migration_owner.preflight(
            migration_candidate(tmp_path), backup_id="verified"
        )
        migration_owner.start(plan["id"], plan["plan_digest"])
        migration_owner.advance(plan["id"], batch_size=1)

        def unavailable(*_args, **_kwargs):
            raise OSError("fake storage timeout")

        with monkeypatch.context() as fault:
            fault.setattr(LocalStorageBackend, "create_stream", unavailable)
            with pytest.raises(OSError, match="fake storage timeout"):
                migration_owner.advance(plan["id"])
        ...
        assert sorted(obj.attempts for obj in objects) == [1, 2]

    def test_cleanup_requires_exact_confirmation(self, active_copy):
        owner, plan, source, key = active_copy

        with pytest.raises(ValueError, match="migration_cleanup_confirmation_required"):
            owner.cleanup(plan["id"], confirmation="other-run", source=True)

        assert source.read_bytes(key) == b"retain"

    def test_source_cleanup_requires_full_audit(
        self, db_session, tmp_path, migration_owner
    ):
        from app.modules.storage.storage_backend.runtime import get_backend
        from tests.factories import build_model, build_stored_file
        ...
        with pytest.raises(ValueError, match="migration_full_audit_required"):
            migration_owner.cleanup(plan["id"], confirmation=plan["id"], source=True)

        assert source.read_bytes(key) == b"source"

    def test_active_destination_cannot_be_discarded(self, active_copy):
        owner, plan, source, key = active_copy

        with pytest.raises(ValueError, match="migration_destination_discard_forbidden"):
            owner.cleanup(plan["id"], confirmation=plan["id"], source=False)

        assert owner.get(plan["id"])["state"] == "active"
        assert source.read_bytes(key) == b"retain"
    ...
    def test_stale_plan_cannot_start(self, db_session, tmp_path, migration_owner):
        result = migration_owner.preflight(
            migration_candidate(tmp_path), backup_id="verified-backup"
        )
    ...
    def test_source_audit_refuses_missing_primary(
        self, db_session, tmp_path, migration_owner
    ):
        from tests.factories import build_file, build_model

        build_file(
            db_session, build_model(db_session), path=str(tmp_path / "missing.stl")
        )
    ...
    def test_cutover_includes_baseline_ingestion_delta(
        ...
        db_session.expire_all()
        assert (
            get_backend().read_bytes(db_session.get(File, baseline_id).path)
            == b"baseline"
        )
    ...
    @pytest.mark.parametrize(
        "phase", ["draining", "delta_copy", "verifying", "activating"]
    )
    def test_restart_before_activation_requires_explicit_resume(
        self, db_session, tmp_path, migration_owner, monkeypatch, phase
    ):
        from app.modules.storage import migration_journal
        from app.modules.storage.storage_backend.runtime import get_backend
        from tests.factories import build_model, build_stored_file
    ...
    def test_incomplete_native_upload_is_fenced_to_retained_source(
        ...
        assert namespace_ref(manager.adapter_for(fenced).backend) == namespace_ref(
            source
        )
        with pytest.raises(
            ValueError, match="migration_source_upload_cleanup_required"
        ):
            migration_owner.cleanup(plan["id"], confirmation=plan["id"], source=True)
    ...
    def test_cleanup_preserves_changed_source_identity(self, active_copy):
        owner, plan, source, key = active_copy
        path = Path(key)
        path.unlink()
    ...
    def test_explicit_retention_never_deletes_source(
        self, active_copy, db_session, remove_credentials
    ):
        from app.db.models import VaultMigrationRun
    ...
    def test_cleanup_waits_for_old_generation_stream(
        self, db_session, tmp_path, migration_owner
    ):
        from app.modules.storage.storage_backend import generations
        from app.modules.storage.storage_backend.runtime import get_backend
    ...
    def test_discard_deletes_only_receipt_proven_destination_copy(
        ...
        result = migration_owner.cleanup(
            plan["id"], confirmation=plan["id"], source=False
        )
    ...
    def test_full_census_preserves_managed_objects_but_excludes_linked_files(
        ...
        document.body = (
            f"![embedded](/api/v1/documents/{document.id}/images/diagram.png)"
        )
        ...
        slot = build_capture_slot(
            db_session, item, sha256=hashlib.sha256(b"x").hexdigest()
        )
...
class TestVaultGenerationMiddleware:
    @pytest.mark.asyncio
    async def test_http_stream_retains_coherent_source_generation(
    ...
    @pytest.mark.asyncio
    async def test_cutover_drains_background_work_after_response_headers(
        self, db_session
    ):
        import asyncio
        import threading
        ...
        request = asyncio.create_task(
            VaultGenerationMiddleware(application)(
                {
                    "type": "http",
                    "path": "/api/v1/maintenance/audits",
                    "method": "POST",
                },
                receive,
                send,
