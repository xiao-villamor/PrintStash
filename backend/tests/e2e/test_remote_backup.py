...
class TestReplicaRunRecovery:
    ...
    async def test_partial_run_retries_the_same_archive_through_the_api(
        ...
        endpoint = s3_endpoint()
        bucket = f"retry-api-{uuid4().hex[:12]}"
        client = boto3.client(
            "s3",
            endpoint_url=endpoint,
            region_name="us-east-1",
        ...
        try:
            response = await api.post(
                f"/api/v1/backups/runs/destinations/{failed['id']}/retry",
                headers=headers,
            )
            ...
            key = result["key"].removeprefix(f"s3/{bucket}/")
            remote = client.get_object(Bucket=bucket, Key=key)["Body"]
            try:
                assert (
                    hashlib.sha256(remote.read()).hexdigest() == body["archive_sha256"]
                )
            finally:
                remote.close()
