"""Defends ``test_openapi_contract`` behavior for the ``repo`` production unit.
...
CONTRACT_PATH = TEST_DATA_DIR / "openapi_contract.json"
...
def test_openapi_contract() -> None:
    ...
    assert current_contract == expected_contract, (
        ...
        "UPDATE_OPENAPI_CONTRACT=1 uv run pytest tests/repo/test_openapi_contract.py -v"
    )
