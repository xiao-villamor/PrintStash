...
class TestRequiredRust:
    @pytest.mark.parametrize("missing", ["printstash_mesh_native", "broken_dependency"])
    def test_native_import_failure_propagates(self, monkeypatch, missing):
    ...
    def test_missing_native_cannot_return_python_output(
    ...
    def test_python_render_callback_is_not_an_api_option(self):
        with pytest.raises(TypeError, match="rasterise_triangles"):
            render_mesh_thumbnail(None, "model", rasterise_triangles=lambda: None)

    def test_render_views_uses_the_native_shared_job(self):
        import trimesh

        mesh = trimesh.creation.box()
        images = native_rasterizer.render_views(mesh, 32, 24, [None])

        assert len(images) == 1
        assert len(images[0]) == 32 * 24 * 3
