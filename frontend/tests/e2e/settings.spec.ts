...
test("non-Verified trash purge sends a one-shot storage-risk acknowledgement", async ({ page }) => {
  await page.goto("/settings?section=trash");
  await page.getByRole("button", { name: "Purge expired" }).click();

  const dialog = page.getByRole("dialog", { name: "Purge expired trash?" });
  await expect(dialog).toContainText("unguarded storage cannot verify every destructive mutation");
