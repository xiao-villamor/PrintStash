...
- Query filtering ONLY via `app/db/scopes.py` — `live(Model)` / `trashed(Model)`. Never hand-write `deleted_at.is_(None)`.
- `services/trash.py` — single owner of trash lifecycle: soft_delete/restore/purge + hourly `gc_soft_deleted()` (lifecycle.py was folded in).

### Read Models & Storage Seam
- `services/model_views.py` — single owner of Model→schema composition (browse list, detail, export, trash list, stats). Routers stay HTTP-only.
- `services/ingestion.persist_artifact()` — shared core for both ingest paths (version→move→File→thumbnail→Metadata). First G-code on a model is auto-marked `is_recommended` (a model with G-code always has exactly one recommended revision).
- Thumbnails stored as WebP (`thumbnail.to_webp()` is the single conversion seam). `backend.thumbnail_key()` → `{id}.webp`; `legacy_thumbnail_key()` → pre-WebP `{id}.png`, read/delete only. `/files/{id}/thumbnail` falls back to legacy PNG.
- StorageBackend: never `isinstance()` on backend type. Use `direct_path(key)` (Path|None), `local_path(key)` ctx-manager (temp download for S3, self-cleaning), `move_in(src, key)`.
...
- `components/model-detail/` — package, not a file: `index.tsx` controller + tab components (overview/settings/revisions/files/history) + presentation.ts maps. Import path `@/components/model-detail` unchanged.
- Display formatters (bytes, duration, timeAgo, mm/%/g/°C/cost) live ONLY in `lib/format.ts` — never redefine locally.
...
## Changelog & Versioning (KEEP UPDATED)

Single source of truth: `frontend/src/lib/changelog.ts` (`CHANGELOG` array, newest FIRST + `GITHUB_REPO`, `APP_VERSION`). Settings → About tab renders it (`settings-panel.tsx`). NEVER hardcode changelog/version lists elsewhere — import from `@/lib/changelog`.

ALWAYS, when shipping a user-facing change:
1. Add/append bullets to the top `CHANGELOG[0].changes` for the in-progress version.
2. On a version bump, prepend a new `{ version, date, changes }` entry (newest first) and sync version in 3 places:
   - `frontend/package.json`
   - `backend/pyproject.toml`
   - `backend/app/core/config.py` → `app_version`
3. `APP_VERSION` auto-derives from `CHANGELOG[0]`; About tab falls back to it when `/health` version lags (pre-rebuild).

Version string MUST match across all 3 files + changelog. Backend rebuild required for `/health` to report new version.
