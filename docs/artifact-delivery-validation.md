...
|80|accepts request adapters without a base URL|Edge|Internal STL helper supplies only request headers|Passthrough and conditional responses retain compatibility without weakening browser-origin checks|Integration|✅ `backend/tests/integration/modules/ingestion/importer/test_new_features.py::TestStlResponse::test_stl_response_streams_raw_stl_without_buffering`; `::test_stl_response_honours_if_none_match`|
