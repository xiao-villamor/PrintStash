...
integration suites; canonical protocol and handoff assertions live in
`test_artifact_uploads.py`; state, chunk, manager, recovery, S3 unit/contract, and
health suites own their corresponding rows; frontend coordinator/component tests
own rows 36–38; and `uploads.spec.ts` owns the reload/resume browser journey.
