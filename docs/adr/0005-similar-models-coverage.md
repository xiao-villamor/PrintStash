...
| # | Behaviour (test name) | Category | Precondition / input | Observable outcome asserted | Tier | Status |
|---|---|---|---|---|---|---|
| S001 | preserves_source_bytes_during_analysis | Happy | Artifact con Revisions e historial | Hash de bytes y contexto iguales tras run | Integration | ✅ `backend/tests/integration/modules/similarity/test_processing.py::TestProcessing::test_preserves_source_bytes_during_analysis` |
...
| S016 | produces_partial_oversized_stl | Edge | STL por encima del cap | D2/invariantes parciales sin exactitud completa | Integration | ✅ `backend/tests/integration/modules/media/test_existing_testdata_similarity.py::TestExistingModels::test_marks_oversized_benchy_partial` |
| S017 | excludes_partial_from_exact_confirmation | Error | Huella partial con hash parecido | Bulk exact no permitido | Integration | ✅ `backend/tests/integration/modules/similarity/test_publication.py::TestPublication::test_partial_inputs_cannot_publish_exact_proof` |
...
| S022 | requires_verification_after_hash_hit | Error | Colisión de cuantización | No candidato identical 1.0 | Integration | ✅ `backend/packages/printstash-core/tests/mesh/similarity/test_verification.py::TestVerifyMeshes::test_does_not_promote_quantized_collision` |
...
| S025 | retains_quarter_face_decimation | Edge | Decimado a 25% | Remeshed elegible | Integration | ✅ `backend/tests/integration/modules/similarity/test_retrieval.py::TestRetrieval::test_retains_quarter_face_decimation` |
...
| S037 | detects_component_multiset_inclusion | Happy | Parte presente en ensamblaje | component_of con dirección y cantidades | Integration | ✅ `backend/tests/integration/modules/similarity/test_component_evidence.py::TestComponentEvidence::test_identifies_part_inside_mixed_assembly` |
| S038 | detects_plate_multiplicity | Happy | Placa de seis copias | plate_of con k=6 | Integration | ✅ `backend/tests/integration/modules/similarity/test_composition.py::TestComposition::test_detects_six_copy_plate` |
| S039 | preserves_multiple_pair_observations | Edge | Dos Artifacts y varios componentes por Model | Observaciones distintas sin overwrite | Integration | ✅ `backend/tests/integration/modules/similarity/test_publication.py::TestPublication::test_retains_distinct_artifact_observations` |
| S040 | meets_identical_precision | Happy | Held-out CSV con defaults | Precision >=0.99; critical | Integration | ✅ `backend/tests/integration/modules/similarity/test_calibration.py::TestFrozenCorpus::test_meets_exact_class_precision` |
| S041 | meets_rescaled_precision | Happy | Held-out escalas y distractores | Precision >=0.99; critical | Integration | ✅ `backend/tests/integration/modules/similarity/test_calibration.py::TestFrozenCorpus::test_meets_exact_class_precision` |
| S042 | meets_remeshed_recall | Happy | Held-out remesh/decimation | Recall >=0.95; critical | Integration | ✅ `backend/tests/integration/modules/similarity/test_calibration.py::TestFrozenCorpus::test_meets_surface_class_recall` |
| S043 | meets_repaired_recall | Happy | Held-out repair corpus | Recall >=0.95; critical | Integration | ✅ `backend/tests/integration/modules/similarity/test_calibration.py::TestFrozenCorpus::test_meets_surface_class_recall` |
| S044 | rejects_false_identical_washers | Error | Washer collision set | Cero falsos identical; critical | Integration | ✅ `backend/tests/integration/modules/similarity/test_calibration.py::TestFrozenCorpus::test_rejects_false_exact_washers` |
| S045 | requires_algorithm_bump_for_changed_golden | Edge | Salida cambia con versión sin cambio | Golden gate falla | Integration | ✅ `backend/tests/integration/modules/similarity/test_calibration.py::TestFrozenCorpus::test_preserves_versioned_evaluation_golden` |
...
| S054 | resumes_from_committed_checkpoint | Edge | Proceso termina a mitad de página | Nuevo proceso completa pendientes sin perder anteriores | Integration | ✅ `backend/tests/integration/modules/similarity/test_restart.py::TestProcessRestart::test_resumes_after_process_exit` |
| S055 | cancels_between_verification_pairs | Happy | Cancel en run activo | Termina bounded, conserva commits | Integration | ✅ `backend/tests/integration/modules/similarity/test_processing.py::TestProcessingFailures::test_cancels_between_committed_verification_pairs` |
...
| S060 | invalidates_candidates_on_source_lifecycle | Edge | Edit/trash/tombstone/purge parametrizados | freshness stale con razón correcta | Integration | ✅ `backend/tests/integration/modules/similarity/test_publication.py::TestPublication::test_source_lifecycle_preserves_confirmed_evidence` |
...
| S062 | suppresses_rejected_pair_for_same_algorithm | Edge | Rerun/threshold/Space nuevo mismo algoritmo | Par rechazado no reaparece abierto | Integration | ✅ `backend/tests/integration/modules/similarity/test_publication.py::TestPublication::test_rejected_pair_stays_rejected_for_same_algorithm` |
| S063 | labels_reconsidered_pair_on_new_algorithm | Edge | Algoritmo cambia | Nueva propuesta enlaza decisión anterior | Integration | ✅ `backend/tests/integration/modules/similarity/test_publication.py::TestPublication::test_new_algorithm_links_preceding_decision` |
...
| S069 | routes_composition_to_multipart | Happy | component_of/plate_of confirmado | Choices/cantidades creadas; no Family | Integration | ✅ `backend/tests/integration/modules/similarity/test_composition.py::TestComposition::test_detects_six_copy_plate` |
...
| S078 | meets_find_similar_response_budget | Edge | 10k library Pi4 o hardware documentado | Inicial <2s; completion medido separado | E2E | ❌ missing |
...
| S086 | uses_embedding_provider_contract | Happy | Space mesh_view y adapter local o compartido con fixture de contrato | Vectores compatibles con dimensión/recipe sin exigir API de AI Search | Contract | ✅ `backend/tests/integration/modules/inference/test_local.py::TestLocalProvider::test_queries_both_native_towers` |
...
| S092 | localizes_similarity_states | Edge | en/es; partial/stale/unsupported/empty | Mensajes traducidos y accesibles | Frontend unit | ❌ missing |
...
| S096 | queries_local_clip_without_ai_search | Happy | Full con modelo CLIP preplaced y sin buscador híbrido | Consulta text-to-shape devuelve vecinos autorizados por la ruta local | Contract | ✅ `backend/tests/integration/modules/inference/preplaced_clip.py::TestPreplacedClip::test_queries_text_through_standalone_index` |
...
| S099 | preserves_stale_evidence_only_confirmation | Edge | Source hash cambia tras confirm_evidence | Decisión humana conservada; freshness stale; ninguna Family ficticia | Integration | ✅ `backend/tests/integration/modules/similarity/test_publication.py::TestPublication::test_source_lifecycle_preserves_confirmed_evidence` |
...
- Core: 1,676 tests passed; branch-aware coverage 99.12%, five floor tests passed; strict Pyright passed.
...
- Remaining: complete coverage floors, the 10k benchmark, final security diff scan and CI. Process restart, cgroup resource containment and scale/reflection browser tests passed.

Additional repository-model acceptance: `test_existing_testdata_similarity.py`
