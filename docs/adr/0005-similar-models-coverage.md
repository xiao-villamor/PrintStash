...
| S056 | releases_temporary_capacity_after_failure | Error | Fallo tras materialización local/S3 | Reserva y temp liberados | Integration | ❌ missing |
...
| S089 | runs_optional_onnx_cpu_adapter | Happy | Model preplaced amd64/arm64 | Vectores compatibles con Space; lane opt-in | Contract | ❌ missing |
...
| F012 | supports minimum numeric dependency types | Edge | Python 3.11/3.13 lowest-direct | Strict Pyright and unchanged geometry goldens pass | Unit | ❌ missing |
...
| F014 | recognizes quarter-count retessellation | Edge | Reverse subdivision on each held-out real design | Actual face count reaches 25%; remeshed at default confidence | Integration | ❌ missing |
